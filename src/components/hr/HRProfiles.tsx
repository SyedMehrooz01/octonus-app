            ...
            <table className="w-full border-collapse min-w-[800px]">
              <thead>
                <tr className="bg-muted/30 text-left border-b border-border">
                  <th className="px-4 sm:px-6 py-4 text-[10px] sm:text-xs font-black text-muted-foreground uppercase tracking-widest">Employee ID</th>
                  ...
                  <th className="px-4 sm:px-6 py-4 text-[10px] sm:text-xs font-black text-muted-foreground uppercase tracking-widest hidden md:table-cell">Designation & Dept</th>
                  <th className="px-4 sm:px-6 py-4 text-[10px] sm:text-xs font-black text-muted-foreground uppercase tracking-widest text-center hidden sm:table-cell">Status</th>
                ...
                {(filteredStaff ?? []).map((s, idx) => (
                  <tr key={s?.id ?? Math.random()} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-muted/5'} hover:bg-primary/5 transition-colors group`}>
                    <td className="px-4 sm:px-6 py-4 sm:py-5">
                      <span className="inline-flex items-center justify-center px-2 sm:px-3 py-1 rounded-lg bg-primary/5 text-primary font-black text-[10px] sm:text-xs border border-primary/10">
                        {s?.id ?? "N/A"}
                      </span>
                    </td>
                    <td className="px-4 sm:px-6 py-4 sm:py-5">
                      <div className="flex items-center gap-3 sm:gap-4">
                        <div className="h-9 w-9 sm:h-11 sm:w-11 shrink-0 rounded-xl bg-gradient-to-tr from-blue-600 to-blue-400 flex items-center justify-center text-white font-black text-xs sm:text-sm shadow-md shadow-blue-500/20">
                          {(s?.name ?? "U").split(" ").map((n:any) => n[0]).join("").toUpperCase()}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm sm:text-base font-black text-foreground leading-tight group-hover:text-primary transition-colors truncate">{s?.name ?? "Unknown"}</p>
                          <p className="text-[10px] sm:text-[11px] font-bold text-muted-foreground uppercase tracking-tighter mt-0.5 truncate">{s?.email ?? "No Email Address"}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 sm:px-6 py-4 sm:py-5 hidden md:table-cell">
                      <p className="text-[13px] sm:text-sm font-black text-foreground/80 leading-tight truncate">{s?.role ?? "N/A"}</p>
                      <p className="text-[9px] sm:text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-1 truncate">{s?.department ?? "Unassigned"}</p>
                    </td>
                    <td className="px-4 sm:px-6 py-4 sm:py-5 text-center hidden sm:table-cell">
                      <Badge className={`${statusColor(s?.status ?? "inactive")} rounded-lg px-2 sm:px-2.5 py-1 text-[9px] sm:text-[10px] font-black uppercase tracking-tighter border-none`}>
                        {s?.status ?? "inactive"}
                      </Badge>
                    </td>
                    <td className="px-4 sm:px-6 py-4 sm:py-5 text-right">
                      <div className="flex items-center justify-end gap-1 sm:gap-1.5 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-0 sm:-translate-x-2 group-hover:translate-x-0">
                        <Button variant="ghost" size="icon" className="h-8 w-8 sm:h-9 sm:w-9 rounded-xl text-primary hover:bg-primary/5" onClick={() => { setSelectedStaff(s); setShowViewModal(true); }}>
                          <Eye className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" className="h-8 w-8 sm:h-9 sm:w-9 rounded-xl text-slate-500 hover:text-slate-700 hover:bg-slate-100/50" onClick={() => handlePrintCard(s)}>
                          <Printer className="h-4 w-4" />
                        </Button>
                        {canDo("edit") && (
                          <Button variant="ghost" size="icon" className="h-8 w-8 sm:h-9 sm:w-9 rounded-xl text-emerald-600 hover:text-emerald-700 hover:bg-emerald-100/50" onClick={() => { setEditStaff(s); setShowEditModal(true); }}>
                            <Edit className="h-4 w-4" />
                          </Button>
                        )}
                        <Button variant="ghost" size="icon" className="h-8 w-8 sm:h-9 sm:w-9 rounded-xl text-violet-600 hover:text-violet-700 hover:bg-violet-100/50" onClick={() => { setLedgerStaff(s); setShowLedgerModal(true); }}>
                          <FileText className="h-4 w-4" />
                        </Button>
                        {user?.role === "admin" && (
                          <Button variant="ghost" size="icon" className="h-8 w-8 sm:h-9 sm:w-9 rounded-xl text-amber-600 hover:text-amber-700 hover:bg-amber-100/50" onClick={() => { setRightsStaff(s); setShowRightsModal(true); }}>
                            <ShieldCheck className="h-4 w-4" />
                          </Button>
                        )}
                        {canDo("delete") && (
                          <Button variant="ghost" size="icon" className="h-8 w-8 sm:h-9 sm:w-9 rounded-xl text-rose-500 hover:text-rose-600 hover:bg-rose-100/50" onClick={() => setShowDeleteConfirm(s?.id)}>
