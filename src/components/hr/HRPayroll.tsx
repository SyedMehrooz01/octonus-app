...
const HRPayroll = memo(({
  ...
  return (
    <div className="mt-8 space-y-6">
      <div className="flex flex-wrap justify-end bg-white p-6 rounded-2xl border border-border shadow-sm gap-2">
        {canDo("export") && (
          ...
          <table className="w-full border-collapse min-w-[700px]">
            <thead>
              <tr className="bg-muted/30 text-left border-b border-border">
                ...
                <th className="px-4 sm:px-6 py-4 text-[10px] sm:text-xs font-black text-muted-foreground uppercase tracking-widest hidden sm:table-cell">Payroll Month</th>
                <th className="px-4 sm:px-6 py-4 text-[10px] sm:text-xs font-black text-muted-foreground uppercase tracking-widest hidden md:table-cell">Calculated Net Pay</th>
                <th className="px-4 sm:px-6 py-4 text-[10px] sm:text-xs font-black text-muted-foreground uppercase tracking-widest text-center">Status</th>
                  ...
                  <tr key={s?.id || `payroll-${idx}`} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-muted/5'} hover:bg-primary/5 transition-colors group`}>
                    <td className="px-4 sm:px-6 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs sm:text-sm font-black text-foreground">{s?.name ?? "Unknown"}</p>
                          <p className="text-[9px] sm:text-[10px] font-bold text-muted-foreground uppercase tracking-tighter mt-0.5">{s?.id ?? "N/A"}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 sm:px-6 py-5 hidden md:table-cell">
                      <p className="text-sm sm:text-base font-black text-emerald-600">₨ {(netSalary || 0).toLocaleString()}</p>
                    </td>
                    <td className="px-4 sm:px-6 py-5 text-center">
                      <Badge className={`${statusColor(latestPayroll ? (latestPayroll?.status ?? "paid") : "pending")} rounded-lg px-2 sm:px-2.5 py-0.5 sm:py-1 text-[9px] sm:text-[10px] font-black uppercase tracking-tighter border-none`}>
                    ...
                    <td className="px-4 sm:px-6 py-5">
                      <div className="flex flex-wrap justify-end gap-2">
                        {canDo("edit") && (
                          <Button size="sm" variant="outline" className="h-8 sm:h-9 rounded-xl font-bold border-primary/20 text-primary hover:bg-primary hover:text-white transition-all px-2.5 sm:px-4 text-[10px] sm:text-xs" onClick={() => {
                        ...
                        <Button 
                            size="icon" 
                            variant="ghost" 
                            className="h-8 w-8 sm:h-9 sm:w-9 rounded-xl text-blue-500 hover:text-blue-600 hover:bg-blue-50" 
            ...
            <tfoot className="bg-muted/40 border-t-2 border-border">
              <tr>
                <td colSpan={2} className="px-4 sm:px-6 py-4 sm:py-6 text-[10px] sm:text-xs font-black text-muted-foreground uppercase tracking-widest">Aggregate Monthly Payout:</td>
                <td className="px-4 sm:px-6 py-4 sm:py-6" colSpan={3}>
                  <div className="flex justify-start sm:justify-start">
                    <span className="px-4 sm:px-6 py-1.5 sm:py-2.5 rounded-xl bg-emerald-500 text-white font-black text-sm sm:text-lg shadow-lg shadow-emerald-500/20">
                      ₨ {((staff ?? []).reduce((acc, s) => {
